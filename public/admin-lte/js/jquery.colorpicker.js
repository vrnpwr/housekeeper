...
(function( factory ) {
	...
	$.widget("vanderlee.colorpicker", {
		options: {
			...
			buttonImage:		'http://brotherslab.com/housekeeper/assets/img/ui-colorpicker.png',
			...
			if (that.element.is('input') || that.options.inline === false) {
				...
				if (/\bbutton|all|both\b/.test(that.options.showOn)) {
					...
					if (that.options.buttonImageOnly && that.image) {
						...
						that.button = $('<button type="button" class="edit_image"></button>').html(that.image || that.options.buttonText).button();
